...
export const addCollectionMethods = CommentsCollection => {
	const { updateRatingScoreOnDoc } = createRatingScoreService(
		CommentsCollection
	);
	...
	CommentsCollection.editComment = (
		documentId,
		newContent,
		userId,
		anonUserData
	...
	CommentsCollection.removeComment = (documentId, userId, anonUserData) => {
		...
		if (CommentsCollection.remove(removeSelector))
	...
	CommentsCollection.likeComment = (documentId, userId, anonUserData) => {
		...
		const likedDoc = CommentsCollection.findOne({
			_id: documentId,
			likes: { $in: [userId] }
		});
		...
		triggerEvent(
			'comment',
			'like',
			Object.assign({}, likedDoc, findSelector, {
				ratedUserId: userId
			})
		);
	};
	...
	CommentsCollection.dislikeComment = (documentId, userId, anonUserData) => {
		...
		const dislikedDoc = CommentsCollection.findOne({
			_id: documentId,
			dislikes: { $in: [userId] }
		});
		...
		triggerEvent(
			'comment',
			'dislike',
			Object.assign({}, dislikedDoc, findSelector, {
				ratedUserId: userId
			})
		);
	};

	CommentsCollection.starComment = (
		documentId,
		starsCount,
		userId,
		anonUserData
		...
		const starredDoc = CommentsCollection.findOne({
			_id: documentId,
			'starRatings.userId': userId
		});

		if (starredDoc) {
			CommentsCollection.update(
				findSelector,
				{ $pull: { starRatings: { userId } } },
				noOptOptions
			);
		}

		CommentsCollection.update(findSelector, {
			$push: { starRatings: { userId, rating: starsCount } }
		});
		...
		triggerEvent(
			'comment',
			'star',
			Object.assign({}, starredDoc, findSelector, {
				ratedUserId: userId,
				rating: starsCount
			})
		);
	...
	CommentsCollection.findForReference = (referenceId, userId, opts = {}) => {
		...
		return CommentsCollection.find(
			comment.getCommentsSelector(referenceId, userId),
			opts
		);
	};
	...
	CommentsCollection.findOnePublic = (_id, userId) =>
		CommentsCollection.findOne({
			_id,
			...comment.commentStatusSelector(userId)
		});

	CommentsCollection.getCountForReferenceId = (
		referenceId,
		userId,
		anonUserData
	) => {
