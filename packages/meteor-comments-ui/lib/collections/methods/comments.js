...
Meteor.methods({
	...
	'comments/reply/add': function(documentId, replyId, content, anonUserData) {
		return CommentsCollection.addReply(
			documentId,
			replyId,
			content,
			this.userId,
	...
	'comments/reply/edit': function(
		documentId,
		replyId,
		newContent,
		anonUserData
	) {
		return CommentsCollection.editReply(
			documentId,
			replyId,
			newContent,
			this.userId,
	...
	'comments/reply/like': function(documentId, replyId, anonUserData) {
		return CommentsCollection.likeReply(
			documentId,
			replyId,
			this.userId,
			anonUserData
	...
	'comments/reply/dislike': function(documentId, replyId, anonUserData) {
		return CommentsCollection.dislikeReply(
			documentId,
			replyId,
			this.userId,
			anonUserData
	...
	'comments/reply/star': function(
		documentId,
		replyId,
		starsCount,
		anonUserData
	) {
		return CommentsCollection.starReply(
			documentId,
			replyId,
			starsCount,
			this.userId,
	...
	'comments/reply/remove': function(documentId, replyId, anonUserData) {
		return CommentsCollection.removeReply(
			documentId,
			replyId,
			this.userId,
			anonUserData
	...
	'comments/count': function(referenceId, anonUserData) {
		return CommentsCollection.getCountForReferenceId(
			referenceId,
			this.userId,
			anonUserData
		);
...
export const getCount = (id, anonData) =>
	promiseCall('comments/count', id, anonData);
