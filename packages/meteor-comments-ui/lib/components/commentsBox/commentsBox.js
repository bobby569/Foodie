...
const loadMoreDocuments = (function() {
   ...
   */
	return function(tplInstance) {
		...
		const anonUserData = userService.isAnonymous()
...
Template.commentsBox.onCreated(function() {
	Comments.session.setDefault(
		this.data.id + '_sorting',
		Comments.config().sortingOptions[0].value
	);

	this.autorun(() => {
		callWithAnonUserData(Comments.getCount, this.data.id).then(count =>
		...
		if (userService.isAnonymous()) {
			...
			userData._id &&
				userData.salt &&
				this.subscribe('comments/anonymous', userData);
		}
	});
...
Template.commentsBox.helpers(
	_.extend(defaultCommentHelpers, {
		customBoxTemplate() {
			return defaultCommentHelpers.getCustomTemplate(
				'boxTemplate',
				'commentsBox',
				this
			);
...
Template.commentsBox.events({
	...
	'submit .comment-form': async function(e) {
		...
		if (trimmedValue)
			await callWithAnonUserData(Comments.add, eventScope.id, trimmedValue);
	...
	'click .stars-action': async function(event) {
		...
		const starRating = parseInt(
			$(event.target)
				.parents('.stars-action')
				.find('.stars-rating .current-rating').length,
			10
		);
	...
	// 	const newContent = editCommentWrapper
	// 		.find('textarea')
	// 		.val()
	// 		.trim();
