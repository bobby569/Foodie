...
if (Meteor.isClient) {
	...
	Tinytest.add('Comments - get', function(test) {
		...
		test.equal(
			Comments.get('getDoc1').fetch()[0].content,
			'I will like it not'
		);
		test.equal(
			Comments.get('getDoc2').fetch()[0].content,
	...
	Tinytest.addAsync('Comments - Raw Collecton Remove', function(
		test,
		completed
	) {
	...
	Tinytest.addAsync('Comments - Reply', function(test, done) {
		...
		Comments.add('fakeDoc1', 'I liked this').then(function({ _id: docId }) {
			Comments.reply(docId, null, 'Stfu WOW')
				.then(function() {
	...
	*/
	...
	Tinytest.add('Comments - changeSchema', function(test) {
		...
		test.equal(Comments._collection.simpleSchema().schema().file, {
			type: Object,
			max: 15
		});
		...
		test.equal(Comments._collection.simpleSchema().schema().image, {
			type: Object,
			optional: true
		});
