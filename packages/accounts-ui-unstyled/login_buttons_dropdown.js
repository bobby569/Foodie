...
Template._loginButtonsLoggedOutDropdown.events({
	...
	'click #signup-link': function() {
		...
		if (username !== null)
			...
			else document.getElementById('login-email').value = usernameOrEmail;
	...
	'click #back-to-login-link': function() {
		...
		var username = trimmedElementValueById('login-username');
		var email =
			trimmedElementValueById('login-email') ||
		...
		var usernameOrEmailInput = document.getElementById(
	...
	'keypress #login-username, keypress #login-email, keypress #login-username-or-email, keypress #login-password, keypress #login-password-again': function(
		event
	) {
...
Template._loginButtonsChangePassword.events({
	'keypress #login-old-password, keypress #login-password, keypress #login-password-again': function(
		event
	) {
...
var trimmedElementValueById = function(id) {
	var element = document.getElementById(id);
	if (!element) return null;
	else return element.value.replace(/^\s*|\s*$/g, ''); // trim() doesn't work on IE8;
};

var loginOrSignup = function() {
	if (loginButtonsSession.get('inSignupFlow')) signup();
	else login();
};
...
var login = function() {
	...
	if (username !== null) {
		...
		else loginSelector = { username: username };
	} else if (email !== null) {
		if (!validateEmail(email)) return;
		else loginSelector = { email: email };
	} else if (usernameOrEmail !== null) {
		// XXX not sure how we should validate this. but this seems good enough (for now),
		// since an email must have at least 3 characters anyways
		if (!validateUsername(usernameOrEmail)) return;
		else loginSelector = usernameOrEmail;
	} else {
...
var forgotPassword = function() {
	...
	if (email.indexOf('@') !== -1) {
		Accounts.forgotPassword({ email: email }, function(error) {
			if (error)
				loginButtonsSession.errorMessage(error.reason || 'Unknown error');
			else loginButtonsSession.infoMessage('Email sent');
		});
	} else {
