...
var resetPassword = function() {
	...
	var newPassword = document.getElementById('reset-password-new-password')
		.value;
...
Template._configureLoginServiceDialog.events({
	...
	'click #configure-login-service-dialog-save-configuration': function() {
		if (
			...
			var serviceName = loginButtonsSession.get(
			...
			_.each(configurationFields(), function(field) {
				configuration[field.property] = document
					.getElementById('configure-login-service-dialog-' + field.property)
			...
			Accounts.connection.call('configureLoginService', configuration, function(
				error,
				result
			) {
				if (error)
					Meteor._debug(
						'Error configuring login service ' + serviceName,
						error
					);
				else
					loginButtonsSession.set('configureLoginServiceDialogVisible', false);
...
Template._configureLoginServiceDialog.templateForService = function(
	serviceName
) {
	serviceName =
		serviceName ||
		loginButtonsSession.get('configureLoginServiceDialogServiceName');
	// XXX Service providers should be able to specify their configuration
	// template name.
	return Template[
		'configureLoginServiceDialogFor' +
